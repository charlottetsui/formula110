...
with no training-only dependency. It loads the reference checkpoint from
`docs/rl_design.md` section 6, causal test 8
(`experiments/2026-09-01_more-training-seed110/checkpoints/policy_final.pt`,
policy weights only): 0/20 eliminations, 20/20 race wins against both
baselines across all 5 fixed evaluation seeds, ~18-21 m/s controlled top
speed. Always acts deterministically (the policy mean, not a sampled
action) since this is inference, not training.
"""
